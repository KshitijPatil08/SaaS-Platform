    ...
    <div className="p-6 lg:p-8 space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
          {total > 0 ? `${total} customer${total !== 1 ? 's' : ''} tracked` : 'Manage and filter all customer accounts'}
        </p>
      </div>
                    ...
                    >
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          <div className={`w-9 h-9 rounded-full bg-gradient-to-br ${AVATAR_COLORS[i % AVATAR_COLORS.length]} flex items-center justify-center text-white text-xs font-semibold shrink-0`}>
                            {getInitials(account.name)}
                          </div>
                          <div className="min-w-0">
                            <p className="font-medium text-slate-900 dark:text-slate-100 truncate">{account.name}</p>
