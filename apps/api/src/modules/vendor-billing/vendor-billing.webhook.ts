...
router.post('/', async (req: Request, res: Response) => {
  ...
  try {
    // ── Idempotency guard ────────────────────────────────────────────────────
    // Stripe guarantees at-least-once delivery. Claim the event ID before any
    // side effects so concurrent deliveries cannot double-apply the update.
    try {
    ...
    if (claimedEvent) {
      await prisma.processedWebhookEvent.deleteMany({ where: { event_id: event.id } }).catch(() => undefined)
    }
