 ...
 *
 * Idempotency: Each Stripe event ID is recorded in ProcessedWebhookEvent
 * before any side-effects run. Duplicate deliveries are rejected with 200.
 */
...
function resolveCompanyIdFromCustomer(
...
router.post('/', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const alreadyProcessed = await (prisma as any).processedWebhookEvent.findUnique({
    ...
    switch (event.type) {
      ...
      case 'checkout.session.completed': {
        ...
        // Invalidate cached plan gate so next request picks up new tier
        kpiCache.invalidate(`plan_gate_${companyId}`)
      ...
      case 'customer.subscription.updated': {
        ...
        const tier = priceId ? priceIdToTier(priceId, config) : null
        ...
        const expiresAt = sub.cancel_at ? new Date(sub.cancel_at * 1000) : null
    ...
    // Mark event as processed — retries will hit the idempotency guard above
